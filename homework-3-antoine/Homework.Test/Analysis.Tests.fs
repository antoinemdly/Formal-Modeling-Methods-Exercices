...
let ``The test model 1 should contain some specific markings`` () =
    ...
    |> CoverabilityGraph.exists (fun marking ->
        marking.Equals( (Marking.where Model1.P0 Omega (Marking.make [ (Model1.P1, 6) ]))) )
    |> should be True

    graph1
    |> CoverabilityGraph.exists (fun marking ->
        marking.Equals(Marking.make [ (Model1.P0, 0) 
                                      (Model1.P1, 7) ]))
    |> should be True
...
let ``The test model 2 should contain some specific markings`` () =
    ...
    graph2
    |> CoverabilityGraph.exists (fun marking -> marking.Equals( testMarking1 ))
    |> should be True

    graph2
    |> CoverabilityGraph.exists (fun marking -> marking.Equals(testMarking2 ))
    |> should be True
